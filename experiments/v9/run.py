#!/usr/bin/env python3
"""
v8 실험 실행: F11/F12 네트워크 Fault + 확장 네트워크 메트릭 (SOP 기반).

Usage:
    python -m experiments.v9.run
    python -m experiments.v9.run --fault F11 --trial 1
    python -m experiments.v9.run --dry-run
"""
import argparse
import logging
import sys
...
from .engine import RCAEngineV9
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(RESULTS_DIR / "experiment_v9.log"),
    ],
)
logger = logging.getLogger("experiment.v9")


def main():
    parser = argparse.ArgumentParser(description="v8 RCA experiment: F11/F12 network fault + extended network metrics")
    ...
    if args.dry_run:
        engine = None
        retriever = None
        validator = None  # dry-run skips validator (consistent with V8 dry-run semantics)
    else:
        engine = RCAEngineV9(model=args.model, provider=args.provider)
        retriever = KnowledgeRetriever()
        # V9 단일 독립변수: Pre-Trial State Validator
        from scripts.stabilize.state_validator import StateValidator
        validator = StateValidator(ground_truth=ground_truth)
        logger.info("V9: StateValidator initialized")
    ...
    logger.info("v9 experiment: %d trials (%s × %s)", total, faults, trials)
    ...
    logger.info("=" * 60)
    logger.info("v9 experiment complete! %d/%d trials", completed, total)
