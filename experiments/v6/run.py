#!/usr/bin/env python3
"""
v6 실험 실행: SOP-Guided Diagnosis Prompt (V3 fork, prompt only changed).

Usage:
    python -m experiments.v6.run
    python -m experiments.v6.run --fault F1 --trial 3
    python -m experiments.v6.run --dry-run
"""
import argparse
import logging
import sys
...
from .engine import RCAEngineV6
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(RESULTS_DIR / "experiment_v6.log"),
    ],
)
logger = logging.getLogger("experiment.v6")


def main():
    parser = argparse.ArgumentParser(description="v6 RCA experiment: SOP-Guided Diagnosis")
    ...
    if args.dry_run:
        ...
        engine = RCAEngineV6(model=args.model, provider=args.provider)
    ...
    logger.info("v6 experiment: %d trials (%s × %s)", total, faults, trials)
    ...
    logger.info("=" * 60)
    logger.info("v6 experiment complete! %d/%d trials", completed, total)
