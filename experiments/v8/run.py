...
Usage:
    python -m experiments.v8.run
    python -m experiments.v8.run --fault F11 --trial 1
    python -m experiments.v8.run --dry-run
"""
import argparse
import logging
import sys
...
from .engine import RCAEngineV8
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(RESULTS_DIR / "experiment_v8.log"),
    ],
)
logger = logging.getLogger("experiment.v8")
...
def main():
    ...
    if args.dry_run:
        ...
        engine = RCAEngineV8(model=args.model, provider=args.provider)
    ...
    logger.info("v8 experiment: %d trials (%s × %s)", total, faults, trials)
    ...
    logger.info("=" * 60)
    logger.info("v8 experiment complete! %d/%d trials", completed, total)
