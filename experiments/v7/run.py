#!/usr/bin/env python3
"""
v7 실험 실행: SOP + Step 3 Reverse-Tracing + Evidence Multiplicity.

Usage:
    python -m experiments.v7.run
    python -m experiments.v7.run --fault F1 --trial 3
    python -m experiments.v7.run --dry-run
"""
import argparse
import logging
import sys
...
from .engine import RCAEngineV7
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(RESULTS_DIR / "experiment_v7.log"),
    ],
)
logger = logging.getLogger("experiment.v7")


def main():
    parser = argparse.ArgumentParser(description="v7 RCA experiment: SOP + Step 3 Reverse-Tracing")
    ...
    if args.dry_run:
        ...
        engine = RCAEngineV7(model=args.model, provider=args.provider)
    ...
    logger.info("v7 experiment: %d trials (%s × %s)", total, faults, trials)
    ...
    logger.info("=" * 60)
    logger.info("v7 experiment complete! %d/%d trials", completed, total)
