#!/usr/bin/env python3
"""
v3 실험 실행: CoT + Harness (Evaluator + Retry + Evidence Verification).

Usage:
    python -m experiments.v3.run
    python -m experiments.v3.run --fault F1 --trial 3
...
from .engine import RCAEngineV3
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(RESULTS_DIR / "experiment_v3.log"),
    ],
)
...
def main():
    parser = argparse.ArgumentParser(description="v3 RCA experiment: CoT + Harness")
    ...
    if args.dry_run:
        ...
        engine = RCAEngineV3(model=args.model, provider=args.provider)
    ...
    logger.info("v3 experiment: %d trials (%s × %s)", total, faults, trials)
    ...
    logger.info("=" * 60)
    logger.info("v3 experiment complete! %d/%d trials", completed, total)
