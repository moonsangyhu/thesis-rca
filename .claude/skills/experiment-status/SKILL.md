...
> **Superpowers 흐름 위치**: Experiment 트랙 Step 4 `superpowers:executing-plans`의 review checkpoint마다 호출 — trial-batch 단위로 사용자에게 상태를 보고한다. 실험 진행 중 발견된 이슈는 `/experiment-issues`로 자동 기록되어 `superpowers:systematic-debugging` Phase 1(Root Cause Investigation)의 입력이 된다.
