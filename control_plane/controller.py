...
from .errors import CampaignNotFound, InvalidTransition, LockHeld, ManifestValidationError
...
class CampaignController:
    ...
    def _approve_once(self, request: ApprovalRequest) -> dict:
        if not hmac.compare_digest(request.user_id, self.config.allowed_user_id):
            return {"status": "rejected", "reason": "user_not_allowed"}
        if not hmac.compare_digest(request.channel_id, self.config.allowed_channel_id):
