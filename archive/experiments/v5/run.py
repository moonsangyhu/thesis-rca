#!/usr/bin/env python3
"""
v5 실험 실행: 2-stage Symptom Extraction -> Diagnosis.

Usage:
    python -m experiments.v5.run
    python -m experiments.v5.run --fault F1 --trial 3
...
from .engine import RCAEngineV5
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(RESULTS_DIR / "experiment_v5.log"),
    ],
)
...
def main():
    parser = argparse.ArgumentParser(description="v5 RCA experiment: Symptom Extraction -> Diagnosis")
    ...
    if args.dry_run:
        ...
        engine = RCAEngineV5(model=args.model, provider=args.provider)
    ...
    logger.info("v5 experiment: %d trials (%s × %s)", total, faults, trials)
    ...
    logger.info("=" * 60)
    logger.info("v5 experiment complete! %d/%d trials", completed, total)
