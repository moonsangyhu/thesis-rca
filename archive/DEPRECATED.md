...
| **V9** | Pre-Trial State Validator. **한 번도 실행 안 됨**(CSV 빈 파일). validator **코드는 `scripts/stabilize/state_validator.py`로 이미 V2.1(옛 V10)에 흡수**되어 `experiments/v9/` 모듈은 중복. | ⚠️ 단, **`docs/plans/experiment_plan_v9.md`·`docs/plans/review_v9.md`는 제자리 유지** — 살아있는 `state_validator.py`(V2.1 사용)의 설계 문서이기 때문. |

## 본류에 남은 버전 (참고)

V2(공정 baseline), V3(하네스 무효 입증), V6(SOP 실패), V7(네트워크 fault 도입),
V8(환경 오염 발견·가설 기각), V2.1(옛 V10 — 환경 재구축 후 re-baseline, 현재).
