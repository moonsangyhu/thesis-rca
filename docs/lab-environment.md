...
0..100 digits와 `<80%`를 검증한다. SSH management path가 시간 초과하거나 marker를
검증하지 못하면, 인증된 kubelet read-only `/stats/summary`의 fresh nodefs
capacity/available을 같은 `<80%` gate의 fail-closed fallback으로 사용한다. full reset/F8 manifest는 optional
