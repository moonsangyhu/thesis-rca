...
- **2026-08-12/16 실행 결정:** 사용자가 별도 과금 가능성을 차단 사유에서 제외했으므로 본실험은 `paid-overage-user-authorized` 모드로 실행한다. 위 zero-overage 조건은 legacy mode에만 적용한다. paid 본실험은 비결정적 SDK quota 조회를 수행하지 않고 active GitHub login을 시작 및 incident 경계에서 결합하며, manifest에 quota 미조회 사유를 기록한다. 30 AIC session limit과 charge journal은 유지한다.
