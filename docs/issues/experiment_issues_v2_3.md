...
### [ISS-060] Loki query timeout이 빈 정상 로그로 변환되어 runtime baseline을 오염

- **카테고리**: infra / data / code / recovery
- **심각도**: critical (P0)
- **영향**: `v2-3-codex-20260830-primary04`는 F1–F6의 30 incidents·90 rows/raw·1,080 logical calls를 commit한 뒤 운영자가 F7-t1 중 안전 중단했다. F7-t1의 partial attempt/charged 20건을 포함해 attempt/charged는 1,100건이며, 해당 incident result/raw/logical ledger는 commit되지 않았다. F4-t3부터 F6-t5까지 연속 13개 committed incident의 `logs.pod_logs`와 `logs.k8s_events`가 모두 0건이므로 campaign 전체를 primary estimand에서 제외한다.
- **관찰한 사실**: `loki-0`의 local-path PVC는 F4-t3 target인 `yms-proxmox-04`에 고정돼 있다. F4-t3 이후 broad namespace query가 30초 timeout을 냈지만 기존 `LokiCollector._query()`는 모든 예외를 `[]`로 바꾸었고, renderer는 비어 있지 않은 `{"pod_logs": [], "k8s_events": []}` dict만 확인해 incident를 계속 커밋했다. 로컬 port-forward는 TCP listener와 `/ready`가 간헐적으로 살아 있어 기존 health gate도 통과했다. 중단 후 listener를 정확히 교체하자 같은 5분 query는 0.107초·0.020초에 HTTP 200 success로 회복했다.
- **추가 복구 오염**: Primary03에서 남은 `emailservice` Service port 9999도 endpoint가 존재한다는 이유로 preflight와 comprehensive health를 통과했다. F7-t1 복구 뒤 Flux app reconcile이 desired port 5000과 live list merge 과정에서 duplicate `grpc` 오류를 내며 Ready=False가 됐다. exact JSON ports replacement 뒤 Flux app Ready와 comprehensive health `ok=True, issues=[]`를 확인했다.
- **근본 원인**: Loki query 성공 여부를 데이터와 함께 봉인하지 않은 fail-open collector, HTTP query가 아닌 TCP/readiness만 검사한 monitoring health, 그리고 endpoint availability만 보고 Service selector/ports desired-state drift를 검사하지 않은 복구 gate가 결합했다.
