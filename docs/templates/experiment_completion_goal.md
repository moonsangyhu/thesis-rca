...
purpose: "autonomous goal 기능에 넣어 한 실험 버전을 수집→독립 비판 분석→다음 실험 부트스트랩(goal 문서+TickTick 투두)→PR 머지까지 자율 종료시키는 표준 프롬프트"
...
2. results/analysis_{{SLUG}}.md **독립 비판 분석 문서** 작성 — 대화 맥락 없는
   fresh sub-agent가 작성(오케스트레이터 직접 작성 금지, 확증 편향 차단).
   필수: ① 데이터 검증(행수·raw·로그 실측) ② 통계(시스템 A vs B McNemar χ²·p값,
   fault별/카테고리별 정확도, eval 점수 분포) ③ 비판적 회고(타당성 5축 + 위키/repo
   선행연구 대비 비판, 논문 정량 인용) ④ 개선 가설 ⑤ 결론·한계, 미수집 fault 상태.
3. **다음 실험 부트스트랩**: docs/plans/next_experiment_goal_{{NEXT_SLUG}}.md(다음 goal 정리본)
   + 새 세션 시작 [GOAL] 프롬프트 + 그 프롬프트를 TickTick `ai-continue` 투두로 저장.
4. 변경이 feature 브랜치 → 한글 PR → {{종료: rebase 머지까지 자동 | PR 생성까지}} 완료
...
[Phase 2-A] 미완 fault 주입 복구 시도 (체계적 디버깅)
  - 가설 검증식 접근: 먼저 수동으로
      ssh -p 22016 debian@211.62.97.71 'ip -o -4 addr show; sudo -n tc qdisc show dev <iface>'
    를 시도해 SSH 도달성·NOPASSWD sudo·tc 존재·**실제 인터페이스명**을 분리 확인.
    (V2.1 사례: 옛 ens18은 없고 K8s 노드망은 vmbr0였음 — iface명 하드코딩을 의심하라.)
  - 흔한 원인: 포트/방화벽 미개방, SSH 키 미배포, sudo 비밀번호 요구, **iface명 불일치**,
    tc(iproute2) 미설치, **SSH 백그라운드 서브셸 hang**(`& ` 작업의 stdout/stderr를
    /dev/null로 안 끊으면 ssh가 EOF 대기로 hang→타임아웃). 원인을 좁혀 scripts/fault_inject/ 수정.
...
[Phase 3] 검증 + 독립 비판 분석 (DoD 2)
  - 먼저 superpowers:verification-before-completion 으로 데이터 실측(행 수·raw·로그) 확인.
  - **분석은 직접 쓰지 말고 독립 sub-agent를 디스패치한다**(general-purpose, fresh 컨텍스트).
    브리프: "편향 없이 비판적으로 분석하라. 위키(~/ms/wiki/wiki/ 탑다운 조회)와
    repo docs/papers/·docs/surveys/ 선행연구를 인용해 우리 결과를 비판하고 개선점을 도출하라."
  - 산출물: results/analysis_{{SLUG}}.md (DoD 2의 5개 섹션 전부). 통계는 실제 데이터로 계산.

[Phase 4] 다음 실험 부트스트랩 (DoD 3 — 한 턴 종결 게이트)
  - Phase 3 비판에서 도출된 1순위 개선 가설로 다음 실험 goal 정리:
    docs/plans/next_experiment_goal_{{NEXT_SLUG}}.md (이 템플릿 기반 치환본).
  - 새 세션에서 바로 붙여넣을 [GOAL] 시작 프롬프트를 위 문서에 포함하고 사용자에게도 제시.
  - 그 프롬프트를 TickTick `ai-continue` 투두로 저장:
    /tmp/tt-next.json 에 {"title","content","tags":["claude-handoff","thesis-rca"]} 작성 후
    python3 ~/.claude/bin/tt_handoff.py < /tmp/tt-next.json (토큰 Keychain, 화면 출력 금지).
...
- (성공) DoD 1·2·3·4 모두 충족(독립 비판 분석 + 다음 실험 부트스트랩 + TickTick 투두 포함)
  → 최종 결과(정확도·p값·미완 fault 상태 + 다음 실험 goal 한 줄) 요약 보고 후 종료.
- (블록) 랩 클러스터/SSH 지속 불가, OpenAI 키 부재, 또는 Phase 2-A 합리적 시도 후 환경 복구 불가
  → 기존 수집 범위로 독립 비판 분석·다음 부트스트랩·PR{{·머지}}까지 완료하고 "미완 fault
  환경 미달성" 한계를 analysis_{{SLUG}}.md와 PR 본문에 명시한 뒤 종료. (이 경우도 DoD 2·3·4는 충족.)
